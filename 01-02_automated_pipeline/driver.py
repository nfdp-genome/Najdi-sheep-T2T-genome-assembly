...
STEPS_DIR = PIPELINE_ROOT / "scripts"
...
def write_failure_row(failures_tsv: Path, row: dict) -> None:
    failures_tsv.parent.mkdir(parents=True, exist_ok=True)
    exists = failures_tsv.exists()
    with failures_tsv.open("a", newline="") as f:
        w = csv.DictWriter(
            f,
            fieldnames=["timestamp","job_id","assembly_id","step","exit_code","slurm_job_id","log_path","tail_stderr"],
        )
        if not exists:
            w.writeheader()
        w.writerow(row)
...
def slurm_wrap(job: Job, cfg: dict, log_path: Path, modules: Optional[str]) -> str:
...
def run_local(job: Job, log_path: Path, use_docker: bool, docker_image: Optional[str], docker_binds: List[str], cpus: int, memory: str) -> int:
    ...
    if use_docker and docker_image:
        ...
        cmd = [
            ...
            *binds, "-w", str(PIPELINE_ROOT), "-e", "HOME=/tmp",
            docker_image, "bash", "-c",
    ...
    with log_path.open("w") as logf:
        proc = subprocess.run(["bash", str(job.script)], env=env, stdout=logf, stderr=subprocess.STDOUT)
    return proc.returncode
...
def append_qc_bundle(jobs, cfg, workdir, *, job_id, label, fasta, depends_on, qc_subtag):
...
def hic_mapping_job_id(sample: str, phase: str, phases: List[str]) -> str:
    if len(phases) == 1 and phase == "primary":
        return f"hic_mapping_{sample}"
    return f"hic_mapping_{phase}_{sample}"
...
def build_jobs(cfg: dict) -> List[Job]:
    ...
    if run_mode == "selective":
        for i, spec in enumerate(cfg.get("selective", {}).get("jobs", [])):
            ...
            jobs.append(Job(job_id=f"selective_{i}_{step}", step_name=step, script=script, env=env, resources_key=spec.get("resources_key", "qc"), outputs=[], depends_on=[]))
        return jobs
    ...
    for phase in phases:
        ...
        # ------------------------------------------------------------------
        # Hi-C mapping step (NEW): map Hi-C reads to contigs -> sorted BAM
        # ------------------------------------------------------------------
        hic_map_jid = hic_mapping_job_id(sample, phase, phases)
        hic_bam_out = root / "hic_mapping" / f"{sample}_{phase}_hic.sorted.bam"
        hic_workdir = root / "hic_mapping"
        jobs.append(
            Job(
                job_id=hic_map_jid,
                step_name="hic_mapping",
                script=STEPS_DIR / "step_hic_mapping.sh",
                env={
                    "HIC_WORKDIR": str(hic_workdir),
                    "ASM_FASTA": str(contig_fa_p),
                    "HIC_R1": str(Path(inputs["hic_r1"]).resolve()),
                    "HIC_R2": str(Path(inputs["hic_r2"]).resolve()),
                    "HIC_BAM_OUT": str(hic_bam_out),
                    "THREADS": str(resources_for(cfg, "hic_mapping").get("cpus", 40)),
                },
                resources_key="hic_mapping",
                outputs=[hic_bam_out],
                depends_on=primary_graph_deps.copy(),
            )
        )
        hic_bam = str(hic_bam_out)
        hic_dep = [hic_map_jid]

        # QC after contigs (depends on contig step, not hic mapping)
        if stages.get("qc_waves", True):
            ...
            jobs.append(
                Job(
                    ...
                    step_name="qc_bundle",
                    script=STEPS_DIR / "step_qc_bundle.sh",
                    env=_qc_env(cfg, str(contig_fa_p), f"{sample}_{phase}_contigs", qc_dir / "busco", log_root / f"qc_contigs_{phase}", qc_dir / "quast"),
                    resources_key="qc",
                    outputs=[],
                    depends_on=primary_graph_deps.copy(),
                )
        ...
        for sc in scaffolders:
            ...
            # scaffolders now depend on hic_mapping completing
            scaff_deps = hic_dep.copy()
            if sc == "yahs":
        ...
        for sc in scaffolders:
            ...
            for pol in polishers:
                if pol == "racon":
                    ...
                    jobs.append(Job(job_id=jid, step_name="polish_racon", script=STEPS_DIR / "step_polish_racon.sh",
                        env={"THREADS": str(resources_for(cfg, "polish").get("cpus", 40)), "ONT_READS": str(Path(inputs["ont_reads"]).resolve()), "ASM_IN": asm_scaff, "WORK_DIR": str(wdir), "TAG": tag, "FINAL_OUT": str(final_fa)},
                        resources_key="polish", outputs=[final_fa], depends_on=scaff_dep.copy()))
                    polish_outputs.append((sc, "racon", tag, final_fa, jid))
                    append_qc_bundle(jobs, cfg, root, job_id=f"qc_polish_racon_{sc}_{sample}{ps}", label=f"{tag}_racon", fasta=final_fa, depends_on=[jid], qc_subtag=f"post_polish_racon_{phase}_{sc}")
                elif pol == "medaka":
                    jid = f"polish_medaka_{sc}_{sample}{ps}"
                    wdir = root / "03_polish" / "medaka" / sc / sample
                    final_fa = wdir / f"{tag}_medaka_consensus.fasta"
                    jobs.append(Job(job_id=jid, step_name="polish_medaka", script=STEPS_DIR / "step_polish_medaka.sh",
                        env={"THREADS": str(resources_for(cfg, "polish").get("cpus", 40)), "ONT_READS": str(Path(inputs["ont_reads"]).resolve()), "ASM_IN": asm_scaff, "WORK_DIR": str(wdir), "TAG": tag, "FINAL_OUT": str(final_fa), "MEDAKA_MODEL": inputs.get("medaka_model", "r941_min_sup_g507")},
                        resources_key="medaka", outputs=[final_fa], depends_on=scaff_dep.copy()))
                    polish_outputs.append((sc, "medaka", tag, final_fa, jid))
                    append_qc_bundle(jobs, cfg, root, job_id=f"qc_polish_medaka_{sc}_{sample}{ps}", label=f"{tag}_medaka", fasta=final_fa, depends_on=[jid], qc_subtag=f"post_polish_medaka_{phase}_{sc}")
        ...
        for sc, pol, tag, pol_fa, _pj in polish_outputs:
            ...
            for gf in gapfillers:
                ...
                if gf == "ntlink":
                    ...
                    jobs.append(Job(job_id=jid, step_name="gap_ntlink", script=STEPS_DIR / "step_gap_ntlink.sh",
                        env={"THREADS": str(resources_for(cfg, "gapfill").get("cpus", 40)), "NTLINK_BIN": str(inputs.get("ntlink_bin") or "ntLink"), "ASM_IN": asm_in, "ONT_FASTQ": ont, "WORK_DIR": str(wdir), "TAG": tag, "POLISHER": pol, "FINAL_OUT": str(final_out)},
                        resources_key="gapfill", outputs=[final_out], depends_on=pdeps.copy()))
                    append_qc_bundle(jobs, cfg, root, job_id=f"qc_{jid}", label=jid, fasta=final_out, depends_on=[jid], qc_subtag=f"post_{jid}")
                elif gf == "tgsgapcloser":
                    wdir = root / "04_gapfill" / "tgsgapcloser" / pol / sc / sample
                    final_out = wdir / f"{tag}_{pol}_tgsgapcloser.fa"
                    jobs.append(Job(job_id=jid, step_name="gap_tgsgapcloser", script=STEPS_DIR / "step_gap_tgsgapcloser.sh",
                        env={"THREADS": str(resources_for(cfg, "gapfill").get("cpus", 40)), "ASM_IN": str(pol_fa), "ONT_FASTQ": ont, "WORK_DIR": str(wdir), "TAG": tag, "FINAL_OUT": str(final_out)},
                        resources_key="gapfill", outputs=[final_out], depends_on=pdeps.copy()))
                    append_qc_bundle(jobs, cfg, root, job_id=f"qc_{jid}", label=jid, fasta=final_out, depends_on=[jid], qc_subtag=f"post_{jid}")

        if stages.get("funannotate_clean_full_tree", False):
            clean_nodes: List[Tuple[str, Path, List[str]]] = []
            clean_nodes.append((f"{sample}_{phase}_contigs", Path(contig_fa_p), primary_graph_deps.copy()))
            for sc in scaffolders:
                if sc in scaff_paths:
                    clean_nodes.append((f"{sample}_{phase}_hic_{sc}", Path(scaff_paths[sc]), [scaffold_job_id(sc, sample, phase, phases)]))
            ...
            for aid, src, deps in clean_nodes:
                ...
                cjid = f"clean_{aid}"
                jobs.append(Job(job_id=cjid, step_name="funannotate_clean", script=STEPS_DIR / "step_funannotate_clean.sh",
                    env={"GENOME": str(src.resolve()), "OUT_FASTA": str(outc), "FUNANNOTATE_RUNNER": str(inputs.get("funannotate_runner", "micromamba run -n funannotate_clean_env"))},
                    resources_key="funannotate_clean", outputs=[outc], depends_on=deps))
                all_clean_job_ids.append(cjid)
                append_qc_bundle(jobs, cfg, root, job_id=f"qc_{cjid}", label=f"{aid}_cleaned", fasta=outc, depends_on=[cjid], qc_subtag=f"post_clean_{aid}")
    ...
    if stages.get("final_quast_multi", False):
        ...
        jobs.append(Job(job_id="quast_multi_final", step_name="quast_multi", script=STEPS_DIR / "step_quast_multi.sh",
            env={"OUT_DIR": str(workdir / "07_quast_all" / "quast_report"), "THREADS": str(resources_for(cfg, "quast_all").get("cpus", 16)), "QUAST_FASTA_LIST": str(list_file), "QUAST_LABELS_FILE": str(labels_file)},
            resources_key="quast_all", outputs=[workdir / "07_quast_all" / "quast_report" / "report.txt"], depends_on=dep_ids))

    return jobs


def _qc_env(cfg, fa, label, busco_root, log_root, quast_root):
    inputs = cfg["inputs"]
...
def run_pipeline(cfg_path: Path, dry_run: bool) -> int:
    ...
    for wi, wave in enumerate(waves):
        ...
        for job in wave:
            ...
            append_manifest(manifest_path, {"job_id": job.job_id, "step": job.step_name, "log": str(job_log), "ts": _ts()})
            ...
            if mode == "slurm":
                ...
                with tempfile.NamedTemporaryFile(mode="w", suffix=".sbatch", delete=False, dir=str(job_log.parent)) as tf:
                    tf.write(body)
                    sbatch_path = tf.name
                try:
                    out = subprocess.check_output(["sbatch", "--parsable", sbatch_path], text=True).strip()
                ...
                res = resources_for(cfg, job.resources_key)
                rc = run_local(job, job_log, use_docker, image, binds, int(res.get("cpus", 4)), str(res.get("memory", "8G")))

            tail = tail_file(job_log, tail_n)
            if rc != 0:
                log.error("FAILED %s exit=%s log=%s", job.job_id, rc, job_log)
                write_failure_row(failures_tsv, {"timestamp": _ts(), "job_id": job.job_id, "assembly_id": job.env.get("LABEL", job.env.get("TAG", "")), "step": job.step_name, "exit_code": rc, "slurm_job_id": slurm_id, "log_path": str(job_log), "tail_stderr": tail})
    ...
    if not dry_run:
        summ = {"finished_at": _ts(), "total_jobs": len(jobs), "state": state}
...
def _wait_slurm(job_id: str, log: logging.Logger, poll: int = 30) -> int:
    while True:
        proc = subprocess.run(["sacct", "-j", job_id, "--format=State,ExitCode", "--noheader", "--parsable2"], capture_output=True, text=True)
