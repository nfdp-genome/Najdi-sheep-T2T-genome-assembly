...
- `00_genomescope_array.config.sh` — path-free Slurm array job: KMC (k=21) counts per HiFi sample, builds the k-mer histogram, then runs GenomeScope2. All inputs and parameters are read from `config.yaml`, so nothing site-specific is baked into the script.
