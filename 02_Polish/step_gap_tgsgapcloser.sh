#!/usr/bin/env bash
#SBATCH --job-name=step_gap_tgsgapcloser
...
set -euo pipefail
MODULES="minimap2/2.24 racon/1.5.0 samtools/1.16.1"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_gap_tgsgapcloser.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_gap_tgsgapcloser.log'))))
PYEOF
)"
