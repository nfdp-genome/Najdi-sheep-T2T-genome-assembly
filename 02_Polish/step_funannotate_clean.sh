#!/usr/bin/env bash
#SBATCH --job-name=step_funannotate_clean
...
set -euo pipefail
MODULES="funannotate/1.8.15"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_funannotate_clean.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_funannotate_clean.log'))))
PYEOF
)"
