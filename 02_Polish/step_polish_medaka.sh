#!/usr/bin/env bash
#SBATCH --job-name=step_polish_medaka
...
set -euo pipefail
MODULES="minimap2/2.24 samtools/1.16.1 medaka"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_polish_medaka.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_polish_medaka.log'))))
PYEOF
)"
