...
set -euo pipefail
MODULES="bwa/0.7.17 samtools/1.16.1"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_hic_map.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_hic_map.log'))))
PYEOF
)"
