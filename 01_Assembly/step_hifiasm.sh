...
set -euo pipefail
MODULES="hifiasm/0.25.0"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_hifiasm.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_hifiasm.log'))))
PYEOF
)"
