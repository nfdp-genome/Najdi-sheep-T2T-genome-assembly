#!/usr/bin/env bash
#SBATCH --job-name=step_qc_bundle
...
set -euo pipefail
MODULES="quast/5.2.0 busco/5.8.3 mummer"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PIPELINE_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
ORIG_SCRIPT="${PIPELINE_ROOT}/scripts/step_qc_bundle.sh"
...
print('log_file=' + shlex.quote(str(cfg.get('log_file','./logs/step_qc_bundle.log'))))
PYEOF
)"
